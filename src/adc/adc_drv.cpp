...
//!
//------------------------------------------------------------------------------
uint8_t adc_init(uint8_t voltage_ref, bool left_adjust, uint8_t adc_channel)
{
...
uint16_t adc_single_conversion(uint8_t adc_input_pin)
{
    ...
    // --- Enable the ADC
    Enable_adc();
	#warning "stabilizzazione della VREF commentata per risparmiare tempo"
    ...
    // --- Disable the ADC
	#warning "disable_adc commentata per risparmiare tempo"
    //Disable_adc();      
